...
namespace Server.Custom
{
    ...
    public partial class AdvancedPlayerGate : Moongate
    {
        ...
        public int[] gAllSkillCaps = new int[55]
        {
        ...
        private int m_PlayerCountMax = 10;
        ...
        private bool m_TakesBackGivenItems = false;
            ...
            else
            {
                ...
                if (_DoesResurrect && !m.Alive)
                {
                    if (_DoesRename)
                    {
                        string nam = m.Name;
                        int last = nam.Length;
                        if (nam.Substring(last - 2, 2) == " I") nam += "I";
                        else if (nam.Substring(last - 3, 3) == " II") nam += "I";
                        ...
                        {
                            nam = m.Female ? NameList.RandomName("female") : NameList.RandomName("male");
                        }
                        else nam += " II";
                ...
                if (_ChangesSkills)
                {
                    SetSkills(m, gAllSkills);
                }

                if (_ChangesCaps)
                {
                    SetSkillCaps(m, gAllSkillCaps);
                }

                if (_ChangesHue) m.Hue = Hue;
                ...
                if (m_TitleAdds)
                {
                    m.Title = m_TitleToAdd;
                }

                if (m_TakesBackGivenItems)
                {
                    TakeBackGivenItems(m);
                }
        ...
        private void SetSkills(Mobile m, int[] skills)
        {
            for (int i = 0; i < m.Skills.Length && i < skills.Length; ++i)
            {
                m.Skills[i].Base = skills[i];
            }
        }

        private void SetSkillCaps(Mobile m, int[] skillCaps)
        {
            for (int i = 0; i < m.Skills.Length && i < skillCaps.Length; ++i)
            {
                m.Skills[i].Cap = skillCaps[i];
            }
        }

        private void GiveSkillItems(Mobile m)
        {
            for (int i = 0; i < m.Skills.Length && i < gAllSkills.Length; ++i)
            {
        ...
        private void TakeBackGivenItems(Mobile m)
        {
            for (int i = 0; i < MC_ItemsToGive.Count; i++)
            {
        ...
        private void GiveItems(Mobile m)
        {
            for (int i = 0; i < MC_ItemsToGive.Count; i++)
            {
                if (MC_ItemsToGive[i] is Item)
    ...
    public class GateDialogGump : Gump
    {
        ...
        private static readonly string[] stNames = new string[] {
            "Alchemy", "Anatomy", "AnimalLore", "ItemID", "ArmsLore", "Parry", "Begging", "Blacksmith",
            "Fletching", "Peacemaking", "Camping", "Carpentry", "Cartography", "Cooking", "DetectHidden",
            "Discordance", "EvalInt", "Healing", "Fishing", "Forensics", "Herding", "Hiding", "Provocation",
            "Inscribe", "Lockpicking", "Magery", "MagicResist", "Tactics", "Snooping", "Musicianship",
            "Poisoning", "Archery", "SpiritSpeak", "Stealing", "Tailoring", "AnimalTaming", "TasteID",
            "Tinkering", "Tracking", "Veterinary", "Swords", "Macing", "Fencing", "Wrestling", "Lumberjacking",
            "Mining", "Meditation", "Stealth", "RemoveTrap", "Necromancy", "Focus", "Chivalry", "Ninjitsu",
            "Bushido", "SpellWeaving"
        };
